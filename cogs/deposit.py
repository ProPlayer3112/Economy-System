...
class dep(commands.Cog):
    ...
    @bridge.bridge_command(
        name='einzahlen',
        description='Zahle dein Geld in die Bank ein',
        aliases=['dep']
    )
    async def deposit(self, ctx, *, amount=None):
        ...
        emErr.add_field(
            name='Aktuelle Diamanten auf Ihrer Hand:',
            value=f'{eco} {cash:,}'
        )
        ...
        if str(amount) == 'all':
            ...
            emA = discord.Embed(
                description=f'Du hast erfolgreich {eco} **{cash:,}** Diamanten eingezahlt.',
            ...
            async with aiosqlite.connect(self.DB) as db:
                async with db.execute("UPDATE economy SET bank = bank + cash, cash = 0 WHERE user_id = ?",
            ...
            em = discord.Embed(
                description=f'Du hast erfolgreich {eco} **{amount}** Diamanten eingezahlt.',
                color=discord.Color.green(),
                timestamp=datetime.now()
            )
